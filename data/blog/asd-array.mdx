...
Perhatikan, pada fungsi `get`, `Array &arr` berarti bahwa fungsi tersebut menerima referensi ke Array. Penambahan `const` di depannya mengindikasikan bahwa fungsi `get` hanya diizinkan untuk membaca data dari Array. Oleh karena itu, jika fungsi `get` mencoba untuk mengubah data dalam Array, akan terjadi error pada saat kompilasi. Biasanya ini disebut sebagai **Read-Only Reference** atau **Immutable Reference**.
...
Perhatikan, pada fungsi `set` menerima `Array &arr` (tanpa `const`), yang berarti fungsi `set` dapat membaca dan juga mengubah data dalam Array.
...
1. Jika $L_{src} = L_{dst}$, maka semua data di Array sumber akan sama dengan data di Array tujuan.
2. Jika $L_{src} < L_{dst}$, hanya $L_{src}$ data pertama dari Array sumber yang akan disalin ke Array tujuan. Data di Array tujuan mulai dari posisi ke-$L_{src}+1$ hingga $L_{dst}$ tidak akan diubah. Sebagai contoh: `src=[1, 2, 3]` dan `dst=[4, 5, 6, 7, 8]`, setelah operasi `copy(src, dst)`, `src=[1, 2, 3]` dan `dst=[1, 2, 3, 7, 8]`.
3. Jika $L_{src} > L_{dst}$, hanya $L_{dst}$ data dari Array sumber yang akan disalin ke Array tujuan. Sebagai contoh: `src=[1, 2, 3, 4, 5]` dan `dst=[6, 7, 8]`, setelah operasi `copy(src, dst)`, `src=[1, 2, 3, 4, 5]` dan `dst=[1, 2, 3]`.
4. Setelah operasi `copy`, jika terjadi perubahan data di Array sumber, data di Array tujuan tidak seharusnya terpengaruh. Jika perubahan terjadi di `dst`, maka `src` juga tidak boleh berubah.
